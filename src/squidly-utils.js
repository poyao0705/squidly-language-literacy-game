...
function isNonNullObject$1(o) {
	return o != null && typeof o === "object";
}
function isNumber(v) {
	return typeof v === "number";
}
function isNonNaNNumber(v) {
	return isNumber(v) && !isNaN(v);
}
function round(num, dp = 0) {
	const pow = isNonNaNNumber(dp) ? 10 ** dp : 1;
...
function atan(rise, run) {
	...
	const theta = Math.atan(abs(rise) / abs(run));
	const pi = Math.PI;
	if (rise > 0) {
		if (run > 0) {
			return theta;
...
class Vector {
	...
	add(x = 0, y = x) {
		const v = parseVector(x, y);
		if (this.isNaN || v.isNaN) throw new NaNError("Addition");
		return new Vector(this._x + v.x, this._y + v.y);
	}
	sub(x = 0, y = x) {
		const v = parseVector(x, y);
		if (this.isNaN || v.isNaN) throw new NaNError("Subtraction");
		return new Vector(this._x - v.x, this._y - v.y);
	}
	mul(x = 0, y = x) {
		const v = parseVector(x, y);
		if (this.isNaN || v.isNaN) throw new NaNError("Multiplication");
		return new Vector(this._x * v.x, this._y * v.y);
	}
	div(x = 0, y = x) {
		const v = parseVector(x, y);
		if (this.isNaN || v.isNaN) throw new NaNError("Division");
	...
	dot(x = 0, y = x) {
		const v = parseVector(x, y);
		if (this.isNaN || v.isNaN) throw new NaNError("Dot Product");
	...
	addV(d) {
		d = parseNumber(d);
		if (isNaN(d) || this.isNaN) throw new NaNError("Vertical Addition");
		return new Vector(this._x, this._y + d);
	}
	addH(d) {
		d = parseNumber(d);
		if (isNaN(d) || this.isNaN) throw new NaNError("Horizontal Addition");
		return new Vector(this._x + d, this._y);
	}

	grad() {
		if (this.isNaN) throw new NaNError("Gradient", "");

		if (isZero(this._x)) return Infinity;
		return this._y / this._x;
	}
	sqrt() {
		if (this.isNaN) throw new NaNError("Square Root", "");
		return new Vector(sqrt(this._x), sqrt(this._y));
	}
	norm() {
		if (this.isNaN) throw new NaNError("Normal length", "");
		return sqrt(sqr(this._y) + sqr(this._x));
	}
	arg() {
		if (this.isNaN) throw new NaNError("Argument", "");
	...
	rotate(theta) {
		...
		if (this.isNaN || isNaN(theta)) throw new NaNError("Rotation", "");
		return new Vector(
			this._x * cos(theta) - this._y * sin(theta),
			this._x * sin(theta) + this._y * cos(theta),
		);
	}
	lurpTo(v, d) {
		v = parseVector(v);
	...
	distToLine(p1, p2) {
		p2 = parseVector(p2);
		const line = p2.sub(p1).rotate(Math.PI / 2);
		const d = line.dot(this.sub(p1)) / line.norm();
		return abs(d);
	}
	reflect(direction = "V") {
		let newVector = null;
	...
	static intersection(a1, b1, a2, b2, onSegment = true) {
		...
		// m = y/x
		const m1 = b1.sub(a1).grad();
		const m2 = b2.sub(a2).grad();
		// c = y - mx
		const c1 = a1.dot(-m1, 1);
		const c2 = a2.dot(-m2, 1);
...
class SvgPlus extends Root {
	...
	constructor(el) {
		...
		const res = extend(el, proto.constructor);
		if (!res) {
			throw (
				"failed to extend element with constructor chain\n" +
				printChain(el["__+"]) +
				"\n with \n" +
	...
	get bbox() {
		...
		const pos = new Vector(bbox);
		const size = new Vector(bbox.width, bbox.height);
		return [pos, size];
	}
   ...
   */
	 ...
	 */
	makeSVGPoint(...values) {
		const v = parseVector(...values);
		const p = Points.createSVGPoint();
		p.x = v.x;
		p.y = v.y;
		return p;
	}
  ...
  */
	async waveTransition(update, duration = 500, dir = false) {
		...
		return new Promise((resolve, reject) => {
			...
			const next = (t) => {
				...
				const theta = Math.PI * (dt / duration + (dir ? 1 : 0));
				const progress = (Math.cos(theta) + 1) / 2;

				const stop = update(progress);
	...
	static parseSVGString(string) {
		...
		const dsvg = doc.querySelector("svg");
	...
	static defineHTMLElement(classDef, className = null) {
		...
		const setters = classDef.observedAttributes;
		...
		const htmlClass = class extends HTMLElement {
			...
			applyAttributes() {
				for (const setter of setters) {
					const value = this.getAttribute(setter);
					if (value != null) {
						this[setter] = value;
					}
				}
...
class AccessEvent extends Event {
	...
	constructor(eventName, mode, config) {
		...
		if (isAccessEvent(mode)) {
			if (
				mode.initialEvent != null &&
				Array.isArray(mode.initialEvent.eventPromises)
			) {
	...
	async _waitForAll() {
		...
		while (i < this.initialEvent.eventPromises.length) {
			const promise = this.initialEvent.eventPromises[i];
			await promise;
			i++;
		}
	}
...
class AccessButtonsLookupTable {
	...
	getVisibleGroups() {
		...
		for (const name in lookup) {
			const group = lookup[name].filter(
				(button) => button.isConnected && button.isVisible,
			);
			if (group.length > 0) {
				group.sort((a, b) => {
	...
	getVisibleButtonsInGroup(group) {
		const { lookup } = this;
		if (group in lookup) {
			const buttons = lookup[group].filter(
				(button) => button.isConnected && button.isVisible,
			);
			buttons.sort((a, b) => {
				if (a.order != null && b.order == null) return -1;
...
function checkClickable(root, element, center) {
	let clickable = false;
	try {
		const els = root.elementsFromPoint(center.x, center.y);
...
class AccessButtonRoot extends HTMLElement {
	...
	static get observedAttributes() {
		return ["access-group", "access-order"];
	}

	/** @return {string} */
	get group() {
		return $.get(this).group;
	}
	 ...
	 * */
	getIsVisible() {
		return this.isPointInElement(this.center);
	}
	 ...
	 */
	isPointInElement(p) {
		const root = this.hostedRoot;
		const proxy = this.clickBoxElement;
...
for (const name in IconSourceText) {
	try {
		const svgString = IconSourceText[name];
		const svg = SvgPlus.parseSVGString(svgString);
		for (const e of svg.querySelectorAll("defs, style, script")) e.remove();
...
class Icon extends SvgPlus {
	...
	squareViewBox() {
		...
		const newSize = new Vector(maxDim / 0.9);
		const newPos = pos.add(size.sub(newSize).div(2));
	...
	set name(name) {
		...
		this.styleElement = this.createChild("style", {
			content: DEFAULT_ICON_STYLE,
		});
...
function tokeniseMath(text) {
	...
	if (matches.length > 1) {
		for (let i = 0; i < matches.length; i += 2) {
			const open = matches[i].index;
			const close = matches[i + 1].index + 2;
			const token = `!${(new Date()).getTime()}${i}!`;
	...
	adjustFS() {
		if (!this.markdownMode.markdown && !this.markdownMode.math) {
			...
			if (value !== "" && width > 0) {
				...
				const wordWidths = value
					.split(" ")
					.map((w) => Ctx.measureText(w).width);
				const tWidth = Math.max(...wordWidths);
				const dWidth = width * 0.95;

				if (dWidth / tWidth < 1) {
					this.children[0].style.setProperty(
						"font-size",
						`${dWidth / tWidth}em`,
					);
				}
...
function isExactSame(obj1, obj2) {
	if (typeof obj1 !== typeof obj2) return false;
		...
		if (obj1 === null && obj2 === null) return true;
			...
			const k3 = new Set();
			for (const v of k1) k3.add(v);
			for (const v of k2) k3.add(v);
...
function lurp4(x, y, tl, tr, bl, br) {
	const xt = tl.mul(1 - x).add(tr.mul(x));
	const xb = bl.mul(1 - x).add(br.mul(x));
	const p = xt.mul(1 - y).add(xb.mul(y));
	return p;
}
	...
	if (size == 1) {
		...
		for (let y = 0; y < size; y++) {
			for (let x = 0; x < size; x++) {
				const p = lurp4(x * dd, y * dd, tl, tr, bl, br);
				points.push(p);
			}
		}
	}
			...
			while (this.goalValue != this.transValue) {
				...
				const duration =
					this.transValue > this.goalValue
						...
						: this.duration;
				const dv = (t1 - t0) / (1000 * duration);
				t0 = t1;

				const value = this.goalValue;
...
function getQueryKey(string = window.location.search) {
	...
	const key = [...query.entries()].find(
		([k, v]) =>
			k.match(
				/^([-0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz]{20})$/,
			) && v == "",
	);
	const isProxy = query.get("proxy") != null;
	return {
		key: key ? key[0] : null,
		options: query,
		isProxy: isProxy,
...
class PromiseChain {
	...
	async addPromise(func, override = false) {
		const item = { next: null, prom: null, override: override };
...
class PublicProxy extends ProxyClass {
	constructor(instance, restrict) {
		...
		const isPrivate = (prop) => {
			return prop[0] == "_" || prop in restrict || prop[0] == "$";
		};
		super(instance, {
			get: (target, prop, receiver) => {
				if (prop in target) {
					const isF = target[prop] instanceof Function;
					if (isPrivate(prop)) {
						throw new ProxyObjectGetError(
							`Failed to ${isF ? "call" : "get"} ${prop} as it's a private ${isF ? "function" : "property"}.`,
						);
					} else {
					...
					throw new ProxyObjectGetError(
						`No property or function named '${prop}'.`,
					);
				}
			},
				...
				if (prop in instance) {
					if (isPrivate(prop)) {
						throw new PrivacyError(
							`The ${target[prop] instanceof Function ? "function" : "property"} '${prop}' is private.`,
						);
...
const CARD_RENDERERS = {
	...
	plain(size, border = this.BORDER_SIZE) {
		const inSize = size.sub(border);
		const g =
			Math.min(window.innerWidth, window.innerHeight) *
			this.BORDER_RADIUS_PERCENTAGE;
		return `
	...
	folder(size, border = this.BORDER_SIZE) {
		...
		const p0 = new Vector(border / 2, border / 2 + 2 * g);
		const p1 = p0.addV(-g);
		const p2 = p1.add(g, -g);
		...
		const tv = new Vector(t, 0);
		const tv2 = tv.rotate((-Math.PI * 3) / 4);
		...
		const p7 = p1.addH(w - g);
		const p8 = p0.addH(w);

		const rg = new Vector(g);
		const rt = new Vector(t * Math.tan((Math.PI * 3) / 8));

		const tabPath = `M${p0}L${p1}A${rg},0,0,1,${p2}L${p3}A${rt},0,0,1,${p4}L${p5}A${rt},0,0,0,${p6}L${p7}A${rg},0,0,1,${p8}Z`;

		const p9 = p8.addV(h - 3 * g);
		const p10 = p9.add(-g, g);

		const p11 = p10.addH(2 * g - w);
		const p12 = p11.sub(g);

		const card = `M${p8.addV(-0.1)}L${p9}A${rg},0,0,1,${p10}L${p11}A${rg},0,0,1,${p12}L${p0.addV(-0.1)}Z`;
		const outline = `M${p0}L${p1}A${rg},0,0,1,${p2}L${p3}A${rt},0,0,1,${p4}L${p5}A${rt},0,0,0,${p6}L${p7}A${rg},0,0,1,${p8}L${p9}A${rg},0,0,1,${p10}L${p11}A${rg},0,0,1,${p12}Z`;
		return `
...
class GridCard extends SvgPlus {
	...
	constructor(el, type) {
		...
		const rs = new ResizeObserver(this.onresize.bind(this));
		rs.observe(this);
	...
	set type(type) {
		...
		this.cardRenderer = (
			CARD_RENDERERS[theme.card] || CARD_RENDERERS.plain
		).bind(CARD_RENDERERS);
...
class GridIcon extends GridCard {
	...
	makeSubtitleElement() {
		return this.content.createChild(
			MarkdownElement,
			{ class: "subtitle" },
			"div",
		);
	...
	set symbol(symbol) {
		...
		if (symbol !== null && symbol !== undefined) {
			const newSymbol = new GridIconSymbol(symbol);
...
class GridLayout extends SvgPlus {
	 ...
	 */
	add(item, ...posArgs) {
		const [row, col, rowEnd, colEnd] = parseCellPosition(...posArgs);
	...
	addItemInstance(classDef, item, ...posArgs) {
		const instance = new classDef(item);
		...
		if (Array.isArray(items)) {
			...
			if (items.every((i) => SvgPlus.is(i, SvgPlus) || i == null)) {
				...
				valid = items2.every(
					(row) =>
						Array.isArray(row) &&
						row.every((i) => SvgPlus.is(i, SvgPlus) || i == null),
				);
			}

			if (valid) {
				const [rowStart, colStart] = parseCellPosition(...posArgs);
	...
	addItemInstances(classDef, items, ...posArgs) {
		const instances = items.map((item, r) => {
		...
		while (i < words.length) {
			...
			if (width > maxWidth) {
				if (current) {
					...
					splitWord(nextWord, maxWidth, ctx).forEach((line) =>
						lines.push(line),
					);
					current = lines.pop();
					i++;
...
class AccessTextArea extends GridCard {
			...
			events: {
				keyup: (e) => {
					...
					this.updateCaretPosition(
						this.textArea.value.substring(0, caretPosition),
					);
				},
				keydown: (e) => {
					const caretPosition = this.textArea.selectionStart;
	...
	insertSuggestedWord(word, preventEvent = false) {
		...
		const isLastCharSpaceBeforeCaret =
		...
		const newValueUpToCaret =
			valueUpToCaret.substring(0, startOfCurrentWord) + word;
		...
		const valueAfterPostSpace =
			nextSpace === -1 ? valueAfterCaret : valueAfterCaret.substring(nextSpace);
	...
	async updateCaretPosition(force = false) {
		...
		if (text !== this._lastText || force) {
			...
			const wrappedLines = lines.flatMap((l) =>
				wrapText(l, this.textArea.clientWidth, this.ctx),
			);
		...
		let lineCount = 1;
		let subString = "";
		for (const line of this._wrapedLines) {
		...
		if (lineCount !== this._lastLineCount || force) {
			...
			const y = (lineCount - 0.4) * parseFloat(compStyles.fontSize) * 1.5;
			this.styles = {
...
class HideShow extends SvgPlus {
	constructor(el = "div") {
		super(el);
		this.transState = new WaveStateVariable(false, 0.4, (t) => {
	...
	async toggle(isShow, time) {
		...
		if (isShow !== this._shown) {
			...
			if (!time) {
				...
				// Setup animation
				const animation = this.animate(this.animationSequence, {
					duration: time,
					iterations: 1,
					composite: "replace",
					easing: WAVE_CUBIC,
				...
				if (this._animation) {
					const progress =
						...
						this._animation.effect.getComputedTiming().duration;
					animation.currentTime =
						progress * animation.effect.getComputedTiming().duration;
...
class SvgResize extends HideShowTransition {
	...
	addDrawable(drawable) {
		if (
			...
			drawable != null &&
			"draw" in drawable &&
			drawable.draw instanceof Function
		) {
	...
	draw() {
		for (const drawable of this._drawbables) {
			drawable.draw();
		}
	}
	...
	createGrid(gridIntrevals = 5) {
		const grid = this.createChild(Grid);
...
const POINTERS = {
	calibration: class CPointer extends BasePointer {
		constructor(size, cOuter = "red", cInner = "darkred", cText = "white") {
			super();
			const sizeG = this.createChild("g");
			const subG = sizeG.createChild("g", { transform: "translate(-50, -50)" });
			this.circle = subG.createChild("circle", {
				r: 50,
				fill: cOuter,
				cx: 50,
				cy: 50,
			});
			this.circle2 = subG.createChild("circle", {
				r: 10,
				cx: 50,
				cy: 50,
				fill: cInner,
		...
		async showText(duration = 400) {
			await this.tg.show(duration);
		}
		async hideText(duration = 400) {
			await this.tg.hide(duration);
		}
	...
	cursor: class MCursor extends BasePointer {
		...
		set type(type) {
			if (typeof type === "string" && type.length > 1) {
				...
				const b = this.icon.children[0];
				const t = this.icon.children[1];

				const size = 1 + parseInt(type[0]);
	...
	blob: class BPointer extends BasePointer {
		...
		set text(text) {
			...
			if (typeof text === "string" && text.length > 0) {
				const fs = 16;
				const r = (this.size + fs) / Math.sqrt(2);
				const h = fs * (10 / 7);

				const w = text == "host" ? 54 : 98;
		...
		render() {
			for (const g of [this.g, this.g2]) {
				...
				if (this.positionBuffer.length > 0) {
					const col = this.g == g ? "black" : "white";
					const offset = col == "black" ? -1 : 0;
					...
					for (let i = 1; i < this.positionBuffer.length; i++) {
						size /= 1.35;
						const v = this.positionBuffer[i].sub(p0);
						g.createChild("circle", {
							r: size - offset,
							cx: v.x,
							cy: v.y,
							stroke: col,
...
class Grid extends SvgPlus {
	...
	draw() {
		...
		const { W, H } = this.ownerSVGElement;
...
class SvgCanvas extends SvgPlus {
	...
	onconnect() {
		...
		const rel = this.createChild("div", {
			styles: {
				position: "relative",
				display: "inline-flex",
				width: "100%",
	...
	transform(x, y, scale, angle, group) {
		...
		p = p.div(scale);
		p = p.rotate(-angle);
		const transform = `rotate(${(angle * 180) / Math.PI}) scale(${s}) translate(${p.x}, ${p.y})`;
...
class PopUpFrame extends SvgPlus {
	...
	async prompt(message, responses) {
		...
		popup.innerHTML = "";
		popup.createChild("div", { class: "msg", content: message });
		const btns = popup.createChild("div", { class: "btn-box" });
		for (const response of responses) {
			const btn = btns.createChild("div", { class: "btn", content: response });
...
try {
	...
	const styleDump = new SvgPlus("style-dump");
...
class ShadowElement extends SvgPlus {
	...
	async loadStyles(url = this.usedStyleSheets) {
		...
		const styles = await this._stylesProm;
	...
	static async loadStyleSheets(urls = ShadowElement.usedStyleSheets) {
		let styles = [];
		if (typeof urls === "string") urls = [urls];
		if (Array.isArray(urls)) {
			const proms = [...new Set(urls)].map(async (url) => {
				if (!(url in LOADED_STYLES)) {
					const prom = async () => {
						try {
							const res = await fetch(url);
							const text = await res.text();
							const style = await newCSSStyleSheet(text);
							return style;
						} catch (e) {
...
class Rotater extends SlotTransition {
	...
	async _applyTransition(content, immediate = false) {
		const element = immediate ? this.shownSlot : this.hiddenSlot;
		element.innerHTML = "";
		...
		if (!immediate) {
			const lastShown = this.shownSlot;
	...
	get flipped() {
		return !this.flipper.flipped;
	}
	get shownSlot() {
		return this.flipped ? this.slot1 : this.slot2;
	}
	get hiddenSlot() {
		return this.flipped ? this.slot2 : this.slot1;
	}
...
class Slider extends SlotTransition {
	constructor(mode = "vertical") {
		super("div");
		this.class = "slider";
		this.slots = this.createChild(
			HideShowTransition,
			{ class: "slider-transitioner" },
			"div",
			"up",
		);
	...
	async _applyTransition(content, direction = 1) {
		const immediate = !(direction === 1 || direction === -1);

		const element = immediate ? this.slot1 : this.slot2;
		...
		if (!immediate) {
			const [dL, dR] = this._directions;
			const dir = direction > 0 ? dL : dR;
			const opDir = direction > 0 ? dR : dL;
