...
import { defaultQuestionBankQuery, queryClient } from "../queryClient";
import {
	AccessButton,
	GridIcon,
	GridIconSymbol,
...
function getSourceTitle(sourceInfo) {
	return (
		getSourceInfoValue(sourceInfo, "title") ||
		getSourceInfoValue(sourceInfo, "name") ||
		"Question Bank"
	);
}
...
function normalizeQuestion(question, index, sourceInfo = {}) {
	...
	const answerParts = getAnswerParts(question, word).filter(
		(part) => part.length > 0,
	);
	if (!word) word = answerParts.join("");

	const tileValues =
		isObject && Array.isArray(question.tiles)
			? question.tiles.map((tile) => String(tile))
			: getDefaultTiles(answerParts);
	const id =
		isObject && question.id
			? String(question.id)
			: `word-${index + 1}-${word || "empty"}`;

	const rawUnit =
		isObject && question.unit !== undefined ? question.unit : sourceInfo?.unit;

	return {
		id,
		title:
			isObject && question.title
				? String(question.title)
				: getSourceTitle(sourceInfo),
		unit: formatUnit(rawUnit),
		unitKey: normalizeUnitKey(rawUnit) || "all",
		prompt:
			isObject && question.prompt
		...
		utterance:
			isObject && question.utterance ? String(question.utterance) : word,
...
export class WordBuildingGame {
	...
	constructor(
		...
		this.#onGameStateChange =
			onGameStateChange instanceof Function ? onGameStateChange : () => {};
	...
	async render() {
		...
		try {
			const questionBank = await queryClient.ensureQueryData(
				defaultQuestionBankQuery,
			);
			this.#sourceInfo = questionBank.info ?? {};
			const questions = questionBank.questions.map((question, index) =>
				normalizeQuestion(question, index, this.#sourceInfo),
			);
			this.#questions = this.#selectedUnitKey
				? questions.filter(
						(question) => question.unitKey === this.#selectedUnitKey,
					)
				: questions;
	...
	#applyGameState(gameState) {
		...
		this.#questionIndex = Number.isFinite(rawIndex)
			? Math.max(
					0,
					Math.min(Math.round(rawIndex), Math.max(questionCount - 1, 0)),
				)
			: 0;
		this.#viewState =
			gameState?.view === GAME_VIEWS.RESULT
				? GAME_VIEWS.RESULT
				: GAME_VIEWS.PLAY;
		...
		const answers =
			gameState?.answers && typeof gameState.answers === "object"
				? gameState.answers
				: {};
	...
	#allQuestionsCompleted() {
		return (
			...
			this.#questions.every(
				(question) => this.#getQuestionState(question).completed,
			)
		);
	}
	...
	#syncCompletionState() {
		...
		return (
			previousCompletedAt !== this.#completedAt ||
			previousViewState !== this.#viewState
		);
	}
	...
	#createGameLayout(question, state) {
		...
		const { home, previous, clear, next, speaker } = this.#createActionButtons(
			question,
			state,
		);
	...
	#createResultLayout() {
		...
		const { home, previous, clear, restart, speaker } =
			this.#createResultActionButtons();
	...
	#createResultTitleBlock(summary) {
		...
		titleText.append(
			createElement(
				"p",
				"word-prompt",
				`You spelled ${summary.totalWords} ${wordLabel}.`,
			),
		);
	...
	#createTitleBlock(question, state) {
		...
		metaRow.append(
			createElement(
				"p",
				"game-step",
				`Word ${this.#questionIndex + 1} of ${this.#questions.length}`,
			),
		);
		titleText.append(metaRow);
	...
	#createWordIcon(question) {
		...
		if (question.imageUrl) {
			...
			image.alt = question.word
				? `Picture for ${question.word}`
				: "Picture for word";
		...
		wordIcon.addEventListener("access-click", () =>
	...
	#createAnswerSlots(question, state) {
		...
		for (let index = 0; index < question.answerParts.length; index += 1) {
			...
			slot.setAttribute(
				"aria-label",
				selectedTile ? `Remove ${selectedTile.value}` : "Empty letter slot",
			);
			...
			if (selectedTile) {
				...
				slot.addEventListener("access-click", () =>
					this.#removeTileAt(question, index),
				);
	...
	#moveQuestion(step) {
		...
		this.#questionIndex = Math.max(
			0,
			Math.min(nextIndex, this.#questions.length - 1),
		);
		this.#commitGameState();
		this.#renderQuestion();
	}
	...
	#renderEmpty() {
		...
		panel.append(
			...
			createElement(
				"p",
				"empty-message",
				"No words are available in the question bank.",
			),
		);
